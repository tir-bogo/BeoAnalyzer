...
def file_system(tmp_path):
    ...
    with open(main_test_file_path, 'rb') as f_in:
        with gzip.open(main_file2_path, 'wb') as f_out:
    ...
    main_file1_dir = main_dir / main_file1.stem
    ...
    return {
        ...
        "results": {
            "main": {
                "file1": main_file1_dir / temp_file.stem / main_test_file.name,
                "file2": main_dir / main_file2.stem,
                "tmpfile": main_file1_dir / temp_file.name
            },
            "sub": {
...
def test_extract_tar(file_system):
    ...
    file_name = file_system["main"]["file1"].stem
    main_dir = file_system["main"]["dir"]
    expected_dir = (main_dir / file_name).as_posix()
